...
import { Wallet, CheckCircle, AlertCircle, Clock, Home, Users, AlertTriangle } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ setActiveTab, setPaymentPreselectMonth, setIssuePreselectAdd }) => {
  const { currentUser, payments, expenses, issues, apartments, users } = useAppContext();
  ...
  if (isAdmin) {
    const activeTasks = issues.filter(i => i.status === 'in_progress').length;
